...
export class RoleGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        const noAuthRequire = this.reflector.getAllAndOverride<boolean>(NO_AUTH_REQUIRED, [
            context.getHandler(),
            context.getClass(),
        ]);
    ...
    private transformUrl(url: string): string {
        ...
        path = createIndex !== -1 ? path.slice(0, createIndex + '/create'.length) + '/:id' : path;

        const updateIndex = path.indexOf('/update');
        path = updateIndex !== -1 ? path.slice(0, updateIndex + '/update'.length) + '/:id' : path;

        const deleteIndex = path.indexOf('/delete');
        path = deleteIndex !== -1 ? path.slice(0, deleteIndex + '/delete'.length) + '/:id' : path;

        if (queryParamsIndex && createIndex == -1 && updateIndex == -1 && deleteIndex == -1) {
